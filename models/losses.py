...
def binary_cross_entropy2d(input, target, weight=None, size_average=True):
    ...
    input_sigmoid = input_sigmoid.transpose(1, 2).transpose(2, 3).contiguous().view(-1, c)
    input_sigmoid = input_sigmoid[target.view(n * h * w, 1).repeat(1, c) >= 0]
    input_sigmoid = input_sigmoid.view(-1)
    
    mask = target >= 0
    target = target[mask].float()
    loss = F.binary_cross_entropy(input_sigmoid, target,
